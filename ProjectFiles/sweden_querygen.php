...
if(!empty($_POST['city']) && empty($_POST['categ'])){
					...
					$sql = mysql_query("SELECT * FROM sweden WHERE (city='$noval' OR city='$city0' OR city='$city1' OR city='$city2' OR city='$city3')");
					...
					while($row=mysql_fetch_array($sql)){
						...
						<div class='thumbnail'><div class='QViewHover' ><img src='storeimages/sweden/$imgid' alt='$dynamicTitle' />
...
elseif(!empty($_POST['categ']) && empty($_POST['city'])){
					...
					$sql = mysql_query("SELECT * FROM sweden WHERE (categ='$noval' OR categ='$cat0' OR categ='$cat1' OR categ='$cat2' OR categ='$cat3')");
					...
					while($row=mysql_fetch_array($sql)){
						...
						$l_categ = $row["categ"];
						$date_added = strftime("%b %d,20%y",strtotime($row["date_added"]));
						$dynamic_list .= "<div class='col-md-4' style='border:2px solid red;'><a href='sweden_view.php?id=".$id."' class='iframe'>
						<div class='thumbnail'><div class='QViewHover' ><div class='hover' style='border:2px solid green;'>Information</div><img src='storeimages/sweden/$imgid' alt='$dynamicTitle' />
                        <p class='title'>$l_name</p>
                        <p class='price'>+</p>
                        <p class='city'>$l_city</p>
                        <p class='city'>$l_categ</p>
...
elseif(!empty($_POST['city']) && !empty($_POST['categ'])){
					...
					$sql = mysql_query("SELECT * FROM sweden WHERE (city='$noval' OR city='$city0' OR city='$city1' OR city='$city2' OR city='$city3') AND (categ='$noval' OR categ='$cat0' OR categ='$cat1' OR categ='$cat2' OR categ='$cat3')");
...
foreach ($_POST['city'] as $selected){
		$cities.= ucfirst($selected)."&nbsp;&nbsp;";
	}
foreach ($_POST['categ'] as $selected1){
		$cats.= ucfirst($selected1)."&nbsp;&nbsp;";
	}
